...
class dbghelp:
    ...
    def symCallbackProc(self, process, actionCode, callbackData, context):
        with self._lock:
            if actionCode == self.CBA_EVENT or \
                ...
                message = data[0].desc.decode('utf-8').replace("\b", "").strip()
                logger.debug("dllEvent {}>({}) {}".format(self._uniqueProcessHandle, data[0].code, message))
                return 1
    ...
    def loadDll(self):
        try:
            ...
            logger.debug("Loaded dll: {}".format(dllName))
